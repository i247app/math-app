...
import 'package:numi/features/classroom_exercise/screens/teacher_create_classroom_exercise_screen.dart';
import 'package:numi/features/classroom_exercise/screens/teacher_classroom_exercise_detail_screen.dart';
import 'package:numi/features/classroom_exercise/widgets/teacher_list/teacher_assignment_card.dart';
...
import 'package:numi/features/classroom_exercise/widgets/teacher_list/teacher_classroom_exercise_add_button.dart';
import 'package:numi/features/classroom_exercise/widgets/teacher_list/teacher_classroom_exercise_search_field.dart';
import 'package:numi/features/classroom_exercise/widgets/teacher_list/teacher_classroom_exercise_section_header.dart';

class TeacherClassroomExerciseScreen extends StatefulWidget {
  const TeacherClassroomExerciseScreen({
    super.key,
  ...
  @override
  State<TeacherClassroomExerciseScreen> createState() =>
      _TeacherClassroomExerciseScreenState();
}

class _TeacherClassroomExerciseScreenState
    extends State<TeacherClassroomExerciseScreen> {
  ...
  Future<void> _loadExercises({bool forceRefresh = false}) async {
    if (!forceRefresh) {
      final cachedExercises = TeacherClassroomExerciseCache.peekList(
    ...
    try {
      final exercises = await TeacherClassroomExerciseCache.loadList(
  ...
  Future<void> _openCreateClassroomExercise() async {
    final created = await Navigator.of(context).push<bool>(
      MaterialPageRoute<bool>(
        builder: (_) => TeacherCreateClassroomExerciseScreen(
          classroomId: widget.classroomId,
          profileId: widget.profileId,
    ...
    if (created == true) {
      TeacherClassroomExerciseCache.invalidateList(
        classroomId: widget.classroomId,
        profileId: widget.profileId,
        purpose: widget.purpose,
      );
  ...
  void _openExerciseDetail(ClassroomExercise exercise) {
    ...
    TeacherClassroomExerciseCache.seedDetail(
      profileId: widget.profileId,
      exercise: exercise,
    );
                  ...
                  child: Column(
                    ...
                    children: [
                      Align(
                        ...
                        child: TeacherClassroomExerciseAddButton(
                          onTap: _openCreateClassroomExercise,
                        ),
                      ),
                      Padding(
                        padding: const EdgeInsets.only(top: 33),
                        child: TeacherClassroomExerciseSearchField(
                      ...
                      Padding(
                        ...
                        child: TeacherClassroomExerciseSectionHeader(
                          purpose: widget.purpose,
                        ),
                      ),
                      Padding(
