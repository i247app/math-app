...
class _LearningProgressScreenState extends State<LearningProgressScreen> {
  ...
  ExamProgressResponse? _progress;
  ...
  List<ParentAssessmentEntry> get _orderedInitialEntries {
    ...
    entries.sort((a, b) => examDate(a.exam).compareTo(examDate(b.exam)));
    return entries;
  }

  List<ExamProgressPoint> get _points {
    final points = List<ExamProgressPoint>.of(
      _progress?.series ?? const <ExamProgressPoint>[],
    );
    points.sort((a, b) {
  ...
  void didUpdateWidget(covariant LearningProgressScreen oldWidget) {
    ...
    if (oldWidget.profileId != widget.profileId ||
        oldWidget.examService != widget.examService) {
      _progress = null;
      _loadProgress();
    }
  }
  ...
  DateTimeRange _defaultRange() {
    final validDates = _orderedInitialEntries
        .map((entry) => examDate(entry.exam).toLocal())
