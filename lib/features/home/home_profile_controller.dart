...
class HomeProfileState {
  ...
  /// True while [HomeProfileController.switchProfile] is in progress.
  ...
  /// Replaces the `ValueKey` on `HomeRoleDashboard` — tabs react to this
...
///
/// Responsibilities:
/// - [switchProfile]: calls [onActivateProfile], increments [HomeProfileState.profileResetSignal]
/// - [prefetchGrades]: pre-loads grade list for current user
/// - [openMenu] / [closeMenu]: profile switcher menu visibility
/// - [requestAddProfile]: increments [HomeProfileState.openAddProfileRequestId]
///
...
class HomeProfileController extends ChangeNotifier {
  ...
  HomeProfileState _state = const HomeProfileState();
  HomeProfileState get state => _state;
  ...
  /// Increments [HomeProfileState.profileResetSignal] on success so that
  /// [HomeRoleDashboard] resets its data without recreating the widget tree.
  ...
  void _update(HomeProfileState next) {
    _state = next;
    notifyListeners();
  }
