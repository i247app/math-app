...
- Every time an SVG is exported or downloaded from Figma, inspect the saved file before using it in Flutter. Normalize unsupported Figma SVG output such as CSS variable fills (`fill="var(--fill-0, white)"`), percentage-only dimensions (`width="100%" height="100%"`), inline `style` attributes, or other markup that can render blank in `flutter_svg`; prefer explicit numeric `width`/`height`, a valid `viewBox`, and concrete colors such as `fill="white"` or project token colors.
- Profile/avatar UI must check and reuse the shared `ProfileAvatarImage` widget before adding custom image, fallback, or clipping logic, so catalog avatar keys, remote URLs, local paths, borders, and fallbacks stay consistent.
